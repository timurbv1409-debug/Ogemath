...
{
    // Priority 1: рядом с .exe (build/.../debug/data) — это твой основной кейс на Windows
    const QString appDir = QCoreApplication::applicationDirPath();
    QStringList candidates;
    candidates << appDir + "/data/" + fileName
               << appDir + "/" + fileName;

    // Priority 2: варианты вокруг currentPath (IDE / запуск из корня)
...
void ProgressPage::refreshWatcherPaths()
{
    if (!watcher_) return;
    QStringList want;
    want << dataPath("catalog.json") << dataPath("progress.json") << dataPath("sessions.json") << dataPath("submissions.json") ;
