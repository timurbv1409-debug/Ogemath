...
void ProgressPage::reloadAllData()
{
    QString err;
    if (!loadCatalog(&err)) {
        if (tree_) { tree_->clear(); QTreeWidgetItem* it = new QTreeWidgetItem(tree_); it->setText(0, "catalog.json: " + err); }
        return;
    
    // ---- Calendar integration ----
    if (calendarTab_) {
        // load plan.json + settings.json (optional)
        QJsonObject planObj;
        {
            QJsonDocument d;
            QString e;
            const QString p = dataPath("plan.json");
            if (QFile::exists(p) && readJsonFile(p, d, &e)) planObj = d.object();
        }

        int trainingsPerWeek = 3;
        QSet<int> plannedWeekdays;
        {
            QJsonDocument d;
            QString e;
