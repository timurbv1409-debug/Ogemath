...
public:
    // Public wrapper because QCalendarWidget::updateCells() is protected
    void refreshCells() { this->updateCells(); }
    ...
    void setViz(const QMap<QDate, Viz>& v) { viz_ = v; updateCells(); }
...
void CalendarPage::buildUi()
{
    ...
    // На некоторых сборках rich-text + span может выглядеть как "пустые полоски".
    // Делаем максимально надёжно: обычный текст + цветной квадрат символом.
    legendPlanned_->setText(QString::fromUtf8("■ План"));
    legendDone_->setText(QString::fromUtf8("■ Занимался"));
    legendMissed_->setText(QString::fromUtf8("■ Пропуск (был план)"));
    legendMock_->setText(QString::fromUtf8("■ Пробник (рамка)"));

    legendPlanned_->setTextFormat(Qt::PlainText);
    legendDone_->setTextFormat(Qt::PlainText);
    legendMissed_->setTextFormat(Qt::PlainText);
    legendMock_->setTextFormat(Qt::PlainText);

    legendPlanned_->setStyleSheet("color:#b45309;");
    legendDone_->setStyleSheet("color:#15803d;");
    legendMissed_->setStyleSheet("color:#b91c1c;");
    legendMock_->setStyleSheet("color:#1d4ed8;");
...
QString CalendarPage::tooltipForDate(const QDate& d) const
{
    ...
    if (!plan_.isEmpty()) {
        ...
        for (const auto& dv : days) {
            ...
            // прогноз точности из plan items (expectedAccuracy)
            double sumExp = 0.0; int cntExp = 0;
            const QJsonArray items = o.value("items").toArray();
            for (const auto& iv : items) {
                const QJsonObject it = iv.toObject();
                if (it.contains("expectedAccuracy")) { sumExp += it.value("expectedAccuracy").toDouble(); ++cntExp; }
            }
            if (cntExp > 0) {
                const int expPct = (int)qRound(100.0 * (sumExp / (double)cntExp));
                // грубый прогноз качества: 35% объём + 65% ожидаемая точность
                const double vol = qBound(0.0, total / 20.0, 1.0);
                const int q = (int)qRound(100.0 * (0.35*vol + 0.65*(sumExp/(double)cntExp)));
                line += QString::fromUtf8(", прогноз: %1% точность, Q≈%2").arg(expPct).arg(q);
            }
...
void CalendarPage::refreshAll()
{
    ...
    // Не сбрасываем выбранный день на "сегодня" при каждом обновлении.
    if (firstShow_) {
        lastSelectedDate_ = QDate::currentDate();
        firstShow_ = false;
    }
    if (!lastSelectedDate_.isValid()) lastSelectedDate_ = QDate::currentDate();
    showDayDetails(lastSelectedDate_);
}
...
void CalendarPage::paintCalendar()
{
    ...
    // Сохраним выбор пользователя, чтобы обновление ячеек не сбрасывало его.
    const QDate prevSelected = cal_->selectedDate();
    ...
    if (prevSelected.isValid()) cal_->setSelectedDate(prevSelected);
}
